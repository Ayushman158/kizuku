...
Plans produced by `improve-animations`. Each one is self-contained: an executor with no
context from the originating conversation should be able to run it end to end.
...
**002 → 003 → 004 → 001**, by leverage. **002, 003 and 004 are done; 001 remains.**
...
- **001 last.** The biggest change of the four and the only one that alters a *gesture*
  rather than a transition, so it deserves the most feel-checking. It is already prototyped
